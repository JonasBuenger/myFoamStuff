...
#       include "readSIMPLEControls.H"
        ...
        for (int nonOrth=0; nonOrth<=nNonOrthCorr; nonOrth++)
        {
            ...
            myBlockMatrixTools::insertVectorEquation(0, sEqn, blockM, blockX, blockB);
            ...
            (
                  (1/beta) *
                  fvm::ddt(Theta)
                - fvm::laplacian(1/sEqn.A(), Theta)
                  ==
                - fvc::div(fvc::grad(Theta)/sEqn.A())
            );

            blockMatrixTools::insertEquation(3, ThetaEqn, blockM, blockX, blockB);
            ...
            blockMatrixTools::blockRetrieve(0, sx, blockX);
            blockMatrixTools::blockRetrieve(1, sy, blockX);
            blockMatrixTools::blockRetrieve(2, sz, blockX);

            s.internalField().replace(0,sx);
            s.internalField().replace(1,sy);
            s.internalField().replace(2,sz);

            blockMatrixTools::blockRetrieve(3, Theta.internalField(), blockX);
