...
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //
    ...
    for (runTime++; !runTime.end(); runTime++)
    {
        ...
        for (int nonOrth=0; nonOrth<=nNonOrthCorr; nonOrth++)
        {
            ...
            myImplicitDivScheme implDivTheta(s);
            tmp<fvScalarMatrix> mDivThetaX = implDivTheta.coefficients(0);
            tmp<fvScalarMatrix> mDivThetaY = implDivTheta.coefficients(1);
            tmp<fvScalarMatrix> mDivThetaZ = implDivTheta.coefficients(2);

            blockMatrixTools::insertCouplingDiagSource(0,0,mDivThetaX, M, S);
            blockMatrixTools::insertCouplingDiagSource(0,1,mDivThetaY, M, S);
            blockMatrixTools::insertCouplingDiagSource(0,2,mDivThetaZ, M, S);
            blockMatrixTools::insertCouplingUpperLower(0,0,mDivThetaX, M);
            blockMatrixTools::insertCouplingUpperLower(0,1,mDivThetaY, M);
            blockMatrixTools::insertCouplingUpperLower(0,2,mDivThetaZ, M);
