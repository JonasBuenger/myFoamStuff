...
void Foam::myHeatFluxFvPatchVectorField::updateCoeffs()
{
    if (this->updated())
    {
	 ...
	 vectorField normals = patch().nf();

	 // Wärmestrom setzen
	 s = alpha * (Theta_i - Theta_wall)* patch().nf();

	 //Info << "Maximales Theta_i: " << max(Theta_i) << endl;
	 //Info << "Maximales s: " << max(s) << endl;

//	 Info << "alpha \t Theta_i \t Theta_wall \ patch().nf() \t s " << endl;

//	 for (int i=normals.size(); i>normals.size()-10; i--){
//		 Info << alpha[i] << "\t" << Theta_i[i] << "\t" << Theta_wall[i] << "\t" << normals[i] << "\t" << s[i] << endl;
//	 }
